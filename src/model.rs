...
pub type Unfinished = (&'static str, Vector3<f32>, Vector3<Rad<f32>>, Vector3<f32>);
...
impl Finished {
    pub async fn from_unfinished(unfinished_model: Unfinished, device: &wgpu::Device, bind_group_layout: &wgpu::BindGroupLayout) -> Result<LinkedList<Self>, Box<dyn std::error::Error>> {
        let bin = fetch::binary_data(unfinished_model.0).await.unwrap();
        ...
        for model in model_vec {
            ...
            let translation = unfinished_model.1;
            let rotations = unfinished_model.2;
            let scale = unfinished_model.3;
            ...
            let transform_uniform = TransformUniform { inner: transform_matrix.into() };
